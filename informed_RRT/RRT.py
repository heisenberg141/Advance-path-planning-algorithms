...
class RRT:
    ...
    def get_new_point_in_ellipsoid(self, goal_bias, c_best):
        ...
        # Generate a random point in an ellipsoid
            ...
            a10 = (self.goal.row - self.start.row) / c_min
            a11 = (self.goal.col - self.start.col) / c_min
            M = [[a10, 0], [a11, 0]]
            U, S, VT = np.linalg.svd(M)
            # print("U:",U)
            # print("S:",S)
            # print("V:",VT)
            D = [[1, 0], [0, np.linalg.det(U)*np.linalg.det(VT)]]
            C = np.matmul(U, np.matmul(D,VT))

            # Compute diagonal matrix - L
            l1 = c_best / 2
            l2 = (np.sqrt(c_best ** 2 - c_min ** 2)) / 2
            L = [[l1, 0], [0, l2]]

            # Cast a sample from a unit ball - x_ball
            theta = 2*np.pi*np.random.rand()
            R = np.random.rand()
            x_ball = [[R*np.sin(theta)], [R*np.cos(theta)]]

            # Map ball sample to the ellipsoid - x_rand
            point = np.matmul(np.matmul(C,L), x_ball) + x_center
